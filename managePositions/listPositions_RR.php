...
else 
{
	...
	$myIDNumber = $_SESSION['userRecruiterID'];
    $query = "SELECT * FROM vwPositions WHERE (positionIsLocked = 'n' AND (unitOffLimits = 'n' OR unitOffLimits IS NULL))";
    ...
    <table id ="positionlist" class="tablesorter">
        ...
        ?>
            <tr>
				...
				if ($position['positionIsHeld'] == "n" OR $position['positionIsHeldByID'] != $myIDNumber){
                ...
                <td id="note"><? echo "$specialNote  ".$position['positionIsHeldNotes']; ?></td>
                <td id="enlisted" class="yellow"><? 
	                if ($position['positionIsHeldByID'] == $myRSID)
	                {
		                echo strtoupper($position['positionIsHeldEnlisted']);
		            }
		            ?></td>
            </tr>
<? } ?>
