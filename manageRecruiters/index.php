...
<html lang="en">
	<head>
		...
		<link rel="stylesheet" type="text/css" href="./rrncoManager.css" />
...
<body>
		<div id="header"><h1>RRNCO/RSID Manager</h1></div>
		<div id = "mainMenu">
			<? require '../common/mainMenu.php'; ?>			
		</div>
		<div id="toppanel"><?php require 'toppanel_default.php';?></div>
		<div id="divider"></div>
		<div id="bottompanel"><?php require 'rrncoList.php';?></div>
		<div id="footer"><? require '../common/footer.htm'; ?></div>		
<script type="text/javascript">
	window.onresize = function() {setHeight();}
    $(document).ready(function(){
    	setHeight();
		$('#rrncoList').tablesorter({
            headers:{
                0: {sorter:false},
                6: {sorter:false}
                }
            });
            $('#addRRNCO').click(function(event){
                event.preventDefault();
                $('#bottompanel').load('frmCreateRecruiter.php');
            });
            ...
            $('td img[id=edit]').click(function(event){
                event.preventDefault();
                var str = "frmModifyRecruiter.php?id=" + $(this).closest("tr").find("td.hidden").text();
            ...
            $('td img[id=delete]').click(function(event){
                event.preventDefault();
                var str = "frmDeleteRecruiter.php?id=" + $(this).closest("tr").find("td.hidden").text();
                $('#toppanel').load(str);
            });
    });
                function setHeight() {
